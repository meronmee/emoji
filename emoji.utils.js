var EmojiUtils = function () {
	...
	function tranEmoji(str){
		str = '' +str;
		return transOldEmoji(transEmojiAliases(transEmojiYJ(str)));
	}
	function transEmojiYJ(str){
	  var pattern = /\[soon\]\s*([a-zA-Z0-9_+-]+?)\s*\[\/soon\]/gi;
	  str = '' +str;
	  return str.replace(pattern, '<i class="emojiyj emojiyj-$1"></i>');            
	}
	function transOldEmoji(str){
	  var pattern = /\[e\]\s*([a-zA-Z0-9_+-]+?)\s*\[\/e\]/gi;
	  str = '' +str;
	  return str.replace(pattern, '<i class="emoji emoji-$1"></i>').replace(/\[\s*([0-9a-f]+?)\s*\]/gi, '<i class="emoji emoji-$1"></i>');            
	}
	function transEmojiAliases(str){
	  var pattern = /:([a-zA-Z0-9_+-]+?):/gi;
	  str = '' +str;
	  return str.replace(pattern, function ($1, $2) {
			//$1为完整的，$2为括号中的
			if(emojiAliases[$2]){
				return '<i class="emoji emoji-' + $2 + '"></i>'
			} else {
				return $1;
			}
      });            
	...
	function parse(selector){
		...
		for(var i=0; i<elements.length; i++){
			...
			element.innerHTML = tranEmoji(content);
		}
	}
